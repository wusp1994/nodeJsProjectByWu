...
const aboutUsSqlStr = require('../db/sql/AboutUs');
...
router.get('/getAboutUsById',(req, res, next)=>{
    //从连接池中获取连接
    pool.getConnection((error,conn)=>{
        //获取页面传过来的参数
        const id = req.query.id;
        conn.query(aboutUsSqlStr.getAboutUsById,id,(err,result)=>{
            console.log(result);
            if(result.length !==1){
                result = {
                    err_code:1,
...
router.post('/updateAboutUs',(req, res, next)=>{
    //从连接池中获取连接
    pool.getConnection((error,conn)=>{
        conn.query(aboutUsSqlStr.updateAboutUs,[req.body,req.body.id],(err,result)=>{
