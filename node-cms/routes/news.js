...
const newsSQL = require('../db/sql/NewsSQL');
...
router.get('/getNewsList',(req, res, next)=>{
    ...
    pool.getConnection((error,conn)=>{
        console.log("连接成功")
        // console.log(error)
        conn.query(newsSQL.getAllNews,(err,result)=>{
...
router.post('/addNews',(req, res, next)=>{
    ...
    pool.getConnection((error,conn)=>{
        //获取客户端传过来的参数
        const news = req.body;
        conn.query(newsSQL.addNews,news,(err,result)=>{
            //console.log(err);
 ...
 */
router.post('/updateNewsById',(req, res, next)=>{
    //从连接池中获取连接
    pool.getConnection((error,conn)=>{

        conn.query(newsSQL.updateNewsById,[req.body, req.body.id],(err,result)=>{
            //console.log(err);
            //console.log('result',result);
            if(typeof result === 'undefined'||result.affectedRows !==1||err){
                result = {
 ...
 */
router.post('/deleteNewsById',(req, res, next)=>{
    //从连接池中获取连接
    pool.getConnection((error,conn)=>{
        conn.query(newsSQL.deleteNewsById,req.body.id,(err,result)=>{
            //console.log(err);
            //console.log('result',result);
            if(typeof result === 'undefined'||result.affectedRows !==1||err){
                result = {
...
router.get('/getNewsById',(req, res, next)=>{
    //从连接池中获取连接
    pool.getConnection((error,conn)=>{
        conn.query(newsSQL.getNewsById,req.query.id,(err,result)=>{
            //console.log(err);
            //console.log('result',result);
            if(result.length !==1){
                result = {
